...
void display_asc(struct node *root);
void DelNode(struct node **root,char *x);
void search(char *x);
enum KeyStatus ins(struct node *ptr, char *key, char *upKey,char *upSname,char *upStdBday,char *upDeg,char *upStdMajor,int *upUnits,struct node **newnode,info studRec);
...
void insertStudRec(info y){
       ...
       strcpy(key,y.stdno);
       value = ins(root, key, upKey,upSname,upStdBday,upDeg,upStdMajor,&upUnits, &newnode, y);
...
enum KeyStatus ins(struct node *ptr, char *key, char *upKey,char *upSname,char *upStdBday,char *upDeg,char *upStdMajor,int *upUnits,struct node **newnode,info studRec){
   ...
   //char lastKey[11];
    ...
    char newKey[11];
		...
		value = ins(ptr->p[pos], key, newKey,nSname,nStdBday,nDeg,nStdMajor,&nUnits, &newPtr, studRec);
