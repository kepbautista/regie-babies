...
enum KeyStatus ins(struct node *ptr, int *key, int *upKey, char *upAcadYear, char *upCourseNo, char *upSection, char *upTime, int *upMaxStud, enum Semester *upSemOffered, struct node **newnode,info course_offering_rec);
...
void insertCourseOfferingRec(info y){
       ...
       key = y.id;
       value = ins(root, &key, &upKey, upAcadYear, upCourseNo, upSection, upTime, &upMaxStud, &upSemester, &newnode, y);
       ...
       if (value == InsertIt){
          ...
          strcpy(root->course_offering_rec[0].course_no,upCourseNo);
          strcpy(root->course_offering_rec[0].acad_year,upAcadYear);
          strcpy(root->course_offering_rec[0].section,upSection);
          strcpy(root->course_offering_rec[0].time,upTime);
          root->course_offering_rec[0].max_stud = upMaxStud;
          root->course_offering_rec[0].semester = upSemester;
...
enum KeyStatus ins(struct node *ptr, int *key, int *upKey, char *upAcadYear, char *upCourseNo, char *upSection, char *upTime, int *upMaxStud, enum Semester *upSemester, struct node **newnode,info course_offering_rec){
    ...
    enum KeyStatus value;
      ...
      strcpy(upAcadYear,course_offering_rec.acad_year);
      strcpy(upCourseNo,course_offering_rec.course_no);
      strcpy(upSection,course_offering_rec.section);
      strcpy(upTime,course_offering_rec.time);
      *upMaxStud=course_offering_rec.max_stud;
      *upSemester=course_offering_rec.semester;
      ...
      return InsertIt;
		...
		value = ins(ptr->p[pos], key, &newKey, nacadyear, ncourseno, nsection, ntime, &nmaxstud, &nsemester, &newPtr, course_offering_rec);
...
//If keys in node is less than M-1 where M is order of B tree
		if (n < M - 1){
			...
			strcpy(ptr->course_offering_rec[pos].course_no,ncourseno);
			strcpy(ptr->course_offering_rec[pos].acad_year,nacadyear);
      strcpy(ptr->course_offering_rec[pos].section,nsection);
      strcpy(ptr->course_offering_rec[pos].time,ntime);
			ptr->course_offering_rec[pos].max_stud = nmaxstud;
      ptr->course_offering_rec[pos].semester = nsemester;
			ptr->p[pos+1] = newPtr;
...
//If keys in nodes are maximum and position of node to be inserted is last
    ...
    *upKey = ptr->course_offering_rec[splitPos].id;	
    strcpy(upAcadYear,ptr->course_offering_rec[splitPos].acad_year);
    strcpy(upCourseNo,ptr->course_offering_rec[splitPos].course_no);
    strcpy(upSection,ptr->course_offering_rec[splitPos].section);
    strcpy(upTime,ptr->course_offering_rec[splitPos].time);
    *upMaxStud = ptr->course_offering_rec[splitPos].max_stud;
    *upSemester = ptr->course_offering_rec[splitPos].semester;
...
void saveStData(FILE *fp,struct node *ptr){
  int i, j;
    if(ptr){
    for (i=0; i < ptr->n; i++){
              fprintf(fp,"%d|%s|%s|%s|%s|%d|%d\n",ptr->course_offering_rec[i].id,ptr->course_offering_rec[i].acad_year,ptr->course_offering_rec[i].course_no,ptr->course_offering_rec[i].section,ptr->course_offering_rec[i].time,ptr->course_offering_rec[i].max_stud, ptr->course_offering_rec[i].semester);
